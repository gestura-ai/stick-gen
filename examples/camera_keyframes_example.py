...
from src.data_gen.camera import (
    Camera, CameraState, StaticCamera, Pan, Zoom, TrackingCamera
)
from src.data_gen.schema import (
    Scene, Actor, ActorType, Position, ActionType, CameraKeyframe
)
...
def create_demo_scene() -> Scene:
    ...
    hero = Actor(
        ...
        color="blue",
        actions=[(0.0, ActionType.WALK), (4.0, ActionType.WAVE), (6.0, ActionType.JUMP)]
    )
...
def demo_camera_keyframes():
    ...
    for kf in scene.camera_keyframes:
        print(f"    Frame {kf.frame}: pos=({kf.x:.1f}, {kf.y:.1f}), "
...
def demo_camera_controller():
    ...
    for t in [0.0, 1.5, 3.0, 4.0, 6.0, 8.0]:
        ...
        print(f"    t={t:.1f}s: pos=({camera.state.x:.2f}, {camera.state.y:.2f}), "
...
def main():
    ...
    parser.add_argument("--output", type=str, default=None,
                       help="Optional: render to video file")
