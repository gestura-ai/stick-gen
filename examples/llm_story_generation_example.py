...
from src.data_gen.llm_story_engine import (
    LLMStoryGenerator, ScriptSchema, MockBackend, LLM_AVAILABLE
)
...
def demo_script_to_scenes(prompt: str, provider: str = "mock"):
    ...
    for i, scene in enumerate(scenes, 1):
        ...
        for actor in scene.actors:
            print(f"     - {actor.id}: {len(actor.actions)} actions, color={actor.color}")
...
def main():
    ...
    parser.add_argument("--prompt", type=str, default="A ninja sneaking into a bank",
    ...
    parser.add_argument("--output", type=str, default=None,
                       help="Optional: save script as JSON")
