...
from src.data_gen.schema import (
    ActionType, ACTION_TO_IDX, IDX_TO_ACTION, NUM_ACTIONS,
...
def test_model_architecture():
    ...
    model = StickFigureTransformer(
        input_dim=10, d_model=384, nhead=12, num_layers=8,
        output_dim=10, embedding_dim=1024, dropout=0.1, num_actions=NUM_ACTIONS
    )
...
def test_forward_pass():
    ...
    with torch.no_grad():
        outputs = model(motion, text_embedding, return_all_outputs=True, action_sequence=action_sequence)
...
def run_all_tests():
    ...
    tests = [test_action_mappings, test_per_frame_actions, test_model_architecture, test_forward_pass]
    passed = 0
    failed = 0
