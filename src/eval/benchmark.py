...
def run_benchmark(checkpoint_path, data_path, output_path, device="cpu", limit=100):
    ...
    with torch.no_grad():
        for item in tqdm(loader):
            ...
            semantic_proxy = torch.nn.functional.mse_loss(generated_motion, real_motion[:, :target_len, :])
            metrics["semantic_match"].append(1.0 / (1.0 + semantic_proxy.item())) # higher is better
    ...
    scorecard = {
        ...
        "Physics Violation Rate (Lower is better)": np.mean(metrics["physics_violations"]),
