...
class AMASSConverter:
    ...
    def convert_sequence(
        self, npz_path: str, target_fps: int = 25, target_duration: float = 10.0
    ) -> torch.Tensor:
        """
        ...
        if model_type == "smplh":
            ...
            body_pose = torch.tensor(
                poses[:, 3:66], dtype=torch.float32
            )  # Body pose (21 joints × 3)
            global_orient = torch.tensor(
                poses[:, :3], dtype=torch.float32
            )  # Root orientation
            left_hand_pose = torch.tensor(
                poses[:, 66:111], dtype=torch.float32
            )  # Left hand (15 joints × 3)
            right_hand_pose = torch.tensor(
                poses[:, 111:156], dtype=torch.float32
