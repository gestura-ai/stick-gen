...
def check_training_status():
    ...
    for proc in psutil.process_iter(
        ...
        try:
            if "python" in proc.info["name"].lower():
                ...
                print(
                    f"  PID {proc.info['pid']}: CPU={proc.info['cpu_percent']:.1f}% "
    ...
    for fname, mtime, size_mb in checkpoint_files[:5]:
        ...
        age_str = (
            f"{age.seconds // 3600}h {(age.seconds % 3600) // 60}m ago"
            if age.days == 0
    ...
    if python_procs and any(p.info["cpu_percent"] > 50 for p in python_procs):
        ...
        print(
            "⚠️  Python process found but low CPU - may be loading data or between epochs"
        )
    else:
