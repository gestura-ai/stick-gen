...
def generate_ascii_chart(
    values: list, title: str, width: int = 50, height: int = 10
) -> str:
...
def format_metrics_table(metrics: dict) -> str:
    ...
    if "diversity" in metrics:
        table.append(
            f"| Diversity Score | {metrics['diversity']['diversity_score']:.4f} |"
        )
...
def generate_report(
    training_history: list, metrics: dict, config: dict = None, variant: str = "base"
) -> str:
...
"""
    ...
    if training_history:
        ...
        best_epoch = next(
            i for i, h in enumerate(training_history) if h["val_loss"] == best_val
        )
...
def main():
    ...
    parser.add_argument(
        "--config", type=str, default=None, help="Path to model config YAML"
    )
    parser.add_argument(
        "--variant",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="reports/training_report.md",
