    ...
    for dataset_dir in sorted(Path(converted_dir).iterdir()):
        if dataset_dir.is_dir():
            ...
            stats[dataset_name] = {
                ...
                "avg_file_size_kb": (
                    (total_size / len(pt_files) / 1024) if len(pt_files) > 0 else 0
                ),
            }
...
def main():
    ...
    for dataset_dir in sorted(amass_dir.iterdir()):
        if dataset_dir.is_dir():
            npz_count = len(
                [f for f in dataset_dir.rglob("*.npz") if f.name != "shape.npz"]
            )
    ...
    report = {
        ...
        "summary": {
            ...
            "avg_file_size_kb": (
                round((total_size_bytes / total_converted_files / 1024), 2)
    ...
    if action_distribution:
        ...
        for action, count in sorted(
            ...
            print(
                f"  {action}: {count:,} sequences ({count/total_converted_files*100:.1f}%)"
            )
