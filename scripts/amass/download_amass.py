...
def main():
    ...
    parser.add_argument('--subset', action='store_true',
                       help='Download small subset for testing')
    parser.add_argument('--output-dir', default='data/amass',
                       help='Output directory (default: data/amass)')
    parser.add_argument('--all', action='store_true',
                       help='Download all datasets (~50GB)')
    ...
    if args.all:
        ...
        datasets_to_download = sorted(DATASETS.keys(), key=lambda x: DATASETS[x]['priority'])[:5]
    ...
    for i, dataset in enumerate(datasets_to_download, 1):
        ...
        print(f"{i}. {dataset:20s} - {info['size_gb']:4.1f} GB - {info['sequences']:5d} sequences")
    print()
