...
def update_model_card_with_metrics(
    model_card_path: str, metrics: dict, version: str = None
):
    ...
    if "action_accuracy" in metrics:
        ...
        content = re.sub(
            r"Action Accuracy:\s*TBD", f"Action Accuracy: {accuracy:.4f}", content
        )

    if "physics" in metrics:
        physics_score = metrics["physics"].get("physics_score", "TBD")
        content = re.sub(
            r"Physics Score:\s*TBD", f"Physics Score: {physics_score:.4f}", content
        )
    ...
    content = re.sub(
        r"last_updated:\s*\d{4}-\d{2}-\d{2}", f"last_updated: {today}", content
    )
...
def create_release_notes(
    version: str, metrics: dict, training_history: list = None
) -> str:
...
def main():
    ...
    parser.add_argument(
        "--version", type=str, default=None, help="Version number (e.g., 1.0.0)"
    )
    parser.add_argument(
        "--metrics", type=str, default=None, help="Path to evaluation_results.json"
    )
    parser.add_argument(
        "--training-history",
        type=str,
    ...
    parser.add_argument(
        "--repo-name", type=str, default=None, help="HuggingFace repo name"
    )
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Prepare files without uploading"
    )
    ...
    if not args.dry_run:
        repo_name = (
            args.repo_name or f"GesturaAI/{VARIANT_CONFIG[args.variant]['repo_suffix']}"
        )
