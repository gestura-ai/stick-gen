...
def physics_loss(physics_output: torch.Tensor, physics_targets: torch.Tensor) -> tuple:
    ...
    momentum_diff = (pred_mx[1:] - pred_mx[:-1]) ** 2 + (pred_my[1:] - pred_my[:-1]) ** 2
...
def train_epoch(model, loader, optimizer, device, config, epoch, grad_accum_steps=32):
    ...
    for batch_idx, (motion, embedding, targets, actions, physics, camera) in enumerate(pbar):
        ...
        outputs = model(motion, embedding, return_all_outputs=True, 
                       action_sequence=actions, camera_data=camera)
...
def main():
    ...
    parser.add_argument("--data-path", type=str, default="data/train_data_final.pt",
    ...
    parser.add_argument("--wandb-project", type=str, default="stick-gen",
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            save_checkpoint(model, optimizer, epoch, val_loss,
                          f"{args.checkpoint_dir}/best_model.pth", model_config)

        # Periodic checkpoint
        if (epoch + 1) % args.save_every == 0:
            save_checkpoint(model, optimizer, epoch, val_loss,
                          f"{args.checkpoint_dir}/checkpoint_epoch_{epoch}.pth", model_config)

    # Save final model
    save_checkpoint(model, optimizer, args.epochs - 1, val_loss,
                   f"{args.checkpoint_dir}/model_checkpoint.pth", model_config)
